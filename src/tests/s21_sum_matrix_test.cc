...
class S21MatrixSumMatrixTest : public ::testing::Test {
 protected:
  void SetUp() override { srand(static_cast<unsigned>(time(0))); }
};

TEST_F(S21MatrixSumMatrixTest, SumMatrix1) {
  int rows = rand() % 100 + 1;
...
TEST_F(S21MatrixSumMatrixTest, SumMatrix2) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  S21Matrix matrix1(rows, columns);
  S21Matrix matrix2(rows, columns);
...
TEST_F(S21MatrixSumMatrixTest, SumMatrix3) {
...
TEST_F(S21MatrixSumMatrixTest, SumMatrix4) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  ...
  EXPECT_NO_THROW(m.SumMatrix(mtx));
  EXPECT_TRUE(check.EqMatrix(m));
}

TEST_F(S21MatrixSumMatrixTest, SumMatrix5) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  S21Matrix m(rows, cols);
  ...
  EXPECT_THROW(m.SumMatrix(mtx), std::invalid_argument);
}

TEST_F(S21MatrixSumMatrixTest, SumMatrix6) {
  S21Matrix matrix1(3, 1);
  S21Matrix matrix2;
